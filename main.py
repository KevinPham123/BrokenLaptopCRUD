...
def create():
    ...
    brokenlaptops = BrokenLaptop.query.all()
    return render_template("create.html",brokenlaptops=brokenlaptops, title= 'New Broken Laptop')
...
def update(laptop_id):
    if request.form:
        ...
        return redirect("/")
    brokenlaptop = BrokenLaptop.query.filter_by(id=laptop_id).first()
    return render_template('update.html', brokenlaptop = brokenlaptop, title = 'Updating Broken Laptop')
